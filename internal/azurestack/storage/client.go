...
func (client Client) ContainersClient(ctx context.Context, account accountDetails) (StorageContainerWrapper, error) {
	accountKey, err := account.AccountKey(ctx, client)
	if err != nil {
		return nil, fmt.Errorf("retrieving Account Key: %s", err)
	}
	...
	shim := NewDataPlaneStorageContainerWrapper(&containersClient)
	return shim, nil
}
